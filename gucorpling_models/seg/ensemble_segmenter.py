...
def choose_ensemble_method(gold_dict, pred_dict, method='mode'):
	...
	for itemid in range(len(gold_dict['gold_test'])):
		if method == "mode":
			ensemble_preds.append(statistics.mode([max_key(pred_dict[x][itemid]) for x in pred_dict.keys() if 'test' in x]))
		elif method == "average":
			ensemble_preds.append(get_polar_lab(statistics.mean([pred_dict[x][itemid]['B'] for x in pred_dict.keys() if 'test' in x])))
...
if __name__ == "__main__":
	...
	p.add_argument("-c", "--corpus",
	               # default="eng.rst.rstdt",
	               default="all")
	opts = p.parse_args()
	
	if opts.corpus == 'all':
		corpora = ["deu.rst.pcc", "eng.pdtb.pdtb", "eng.rst.gum", "eng.rst.rstdt", "eng.sdrt.stac",
		          "eus.rst.ert", "fra.sdrt.annodis", "nld.rst.nldt", "por.rst.cstn", "rus.rst.rrt",
		          "spa.rst.rststb", "spa.rst.sctb", "tur.pdtb.tdb", "zho.pdtb.cdtb", "zho.rst.sctb"]
	else:
		corpora = [opts.corpus]
	...
	for corpus in corpora:
		...
		gold_dict = {"gold_train": read_gold(opts.data_dir + corpus + os.sep + corpus + "_train.tok")[0],
