...
param_dir = script_dir + "params" + os.sep
...
class SubtreeSegmenter:
	...
	def read_data(self,infile,size,as_text,rare_thresh,chosen_feats=None):
		...
		train_feats, vocab, toks, firsts, lasts = read_conll_conn(infile,genre_pat=self.genre_pat,mode="seg",cap=cap,char_bytes=self.lang=="zho",as_text=as_text)
		vocab = Counter(vocab)
	...
	def predict(self, infile, eval_gold=False, as_text=True):
		...
		feats, _, toks, _, _ = read_conll_conn(infile,genre_pat=self.genre_pat,mode="seg",as_text=as_text,char_bytes=self.lang=="zho")
		...
		sys.stderr.write("o Dumping predictions to " + self.name + self.auto + '_' + self.corpus + ".json\n")
		from json import dumps
		proba_str = dumps(str(probas))
...
if __name__ == "__main__":
	...
	p.add_argument("-d","--data_dir",default=os.path.normpath("../../sharedtask2019/data"),help="Path to shared task data folder")
