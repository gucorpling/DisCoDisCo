...
class E2eResolver(Model):
    ...
    def forward(
        ...
        gold_spans : `torch.IntTensor`, required.
            A tensor of shape (batch_size, num_gold_EDUs, 2), representing the inclusive start and end
            indices of gold EDU boundaries. Comes from a `ListField[SpanField]` of indices into the
        ...
        batch_size = gold_spans.size(0)
        document_length = text_embeddings.size(1)
        num_spans = gold_spans.size(1)

        # Shape: (batch_size, document_length)
        text_mask = util.get_text_field_mask(text)

        # Shape: (batch_size, num_spans)
        span_mask = (gold_spans[:, :, 0] >= 0).squeeze(-1)
        ...
        endpoint_span_embeddings = self._endpoint_span_extractor(contextualized_embeddings, gold_spans)
        # Shape: (batch_size, num_spans, emebedding_size)
        attended_span_embeddings = self._attentive_span_extractor(text_embeddings, gold_spans)
