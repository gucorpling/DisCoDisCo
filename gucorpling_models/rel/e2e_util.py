...
def make_coref_instance(
    ...
    for c in gold_clusters:
        for s in c:
            if s not in flattened_spans:
                flattened_spans.append(SpanField(s[0], s[1], text_field))
    fields["gold_spans"] = ListField(flattened_spans)

    """
    The direction and the EDU label will be added to the model after the object function is defined.
    """
    # fields["dir"] = SequenceLabelField(dir)
