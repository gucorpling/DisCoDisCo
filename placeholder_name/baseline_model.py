...
class Disrpt2021Baseline(Model):
    ...
    def forward(
        ...
        encoded_unit1_body = self.encoder(embedded_unit1_body, util.get_text_field_mask(unit1_body))
        encoded_unit1_sentence = self.encoder(embedded_unit1_sentence, util.get_text_field_mask(unit1_sentence))
        encoded_unit2_body = self.encoder(embedded_unit2_body, util.get_text_field_mask(unit2_body))
        encoded_unit2_sentence = self.encoder(embedded_unit2_sentence, util.get_text_field_mask(unit2_sentence))
    ...
    def make_output_human_readable(
        ...
        output_dict['pred_direction'] = [self.vocab.get_token_from_index(int(i), "direction_labels") for i in direction_index]
        relation_index = output_dict['relation_logits'].argmax(-1)
        output_dict['pred_relation'] = [self.vocab.get_token_from_index(int(i), "relation_labels") for i in relation_index]
